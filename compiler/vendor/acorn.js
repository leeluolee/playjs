...
(function(exports) {
  ...
  // `getLineInfo`用于locations option为off的情况下
  var getLineInfo = exports.getLineInfo = function(input, offset) {
    // line下标从1开始
    for (var line = 1, cur = 0;;) {
      // 每次都重置lineBreak的lastIndex
  ...
  // tokenize返回一个函数，每次调用都会吐出一点，如输入`var a = acorn.tokenize("var a= b;"),JSON.stringify(a())`:
  ...
  // tokenize 只是内部 tokenizer的一个展现, 内部搜索不到任何应用 , #IG
  exports.tokenize = function(inpt, opts) {
    input = String(inpt); inputLen = input.length;
    setOptions(opts);
    // 初始化token， 这里所有的tokenEnd、tokenStart都是全局变量，(按作者的介绍是有2-3%的性能提升...)
    initTokenState();
  ...
  // 
  // __注__:这三个都是Number类型，基于在输入字符串中的位置
  var tokPos;
  ...
  // **注**: 如果需要标示location，则进行信息赋值{line:xx, column:xx}
  ...
  function skipLineComment() {
    ...
    // 分别是 10对应\n, 13\r , 8232即上数`var lineBreak = /\r\n|[\n\r\u2028\u2029]/g;`的\u2028  8329是阿拉伯的什么？即\u
  ...
  // into it.
  //
  // All in the name of speed. : 飚英语：以速度之名
  //
  // The `forceRegexp` parameter is used in the one case where the
  // `tokRegexpAllowed` trick does not work. See `parseStatement`.

  // 可以发现这里都是用charCode来进行判断...
  ...
  function readToken(forceRegexp) {
    ...
    // 如果是\开头 或者是字符开头 都read word
    if (isIdentifierStart(code) || code === 92 /* '\' */) return readWord();

    // 否则读取符号
    var tok = getTokenFromCode(code);

    if (tok === false) { //如果 还未找到
      // If we are here, we either found a non-ASCII identifier
      // character, or something that's entirely disallowed.
      var ch = Str/ing.fromCharCode(code);
      //这里可能readWord不成功
  ...
  // finishOp 是每次token读取的终点
  function finishOp(type, size) {
    var str = input.slice(tokPos, tokPos + size);
    tokPos += size;//tokPos上衣
  ...
  // 每次next会吃一次token, 并且重新对lastXX 与XX的一次调换
  function next() {
    lastStart = tokStart;
    lastEnd = tokEnd;
  ...
  // 断言下一个tokenType是不是需要的type, 是则进行next，并且retrun true
  function eat(type) {
    if (tokType === type) {
      next();
      return true;
  ...
  // does not help.
  // 单个表达式的读取, 找到Expression
  function parseStatement() {
    if (tokType === _slash)
      readToken(true);//即那个regExp参数forceRegexp
  ...
  // onComma 与 noIn 用于阻止? Array literals等，或者for in 等对于for的
  function parseExpression(noComma, noIn) {
    var expr = parseMaybeAssign(noIn);
    if (!noComma && tokType === _comma) {//如果noComma 但是tokType === _comma
  ...
  // `isStatement`标示函数是否是以 var a = function 的是形式,即函数声明 或者函数直接量
  // 

  // 解析一个function
  function parseFunction(node, isStatement) {
    if (tokType === _name) node.id = parseIdent();//如果是函数赋值
    else if (isStatement) unexpected();
    else node.id = null;
    node.params = [];
    var first = true;//标志
    expect(_parenL);// 这里会
    while (!eat(_parenR)) {//如果没有吃到} 的话
  ...
  // an array. 
  //
  // 1. `close` is the token type that ends the list, and (比如}、]标示是何种类型的)
  // 2. `allowEmpty` can be turned on to allow subsequent commas with 比如[,,,]
  // 3. `allowTrailingComma` 是否允许结尾的Comma
  //

  function parseExprList(close, allowTrailingComma, allowEmpty) {
    var elts = [], first = true;
    while (!eat(close)) {//如果吃不到close
      if (!first) {//标示是不是刚开始parse
        expect(_comma);
  ...
  // 如果是关键字，在forbidReserved并且liberal为true(如字面量时), 返回tokType的keyword 
...
// 资源: 
// 
// 1. http://people.mozilla.org/~jorendorff/es5.html es5的规范描述
// 2. https://developer.mozilla.org/en-US/docs/SpiderMonkey/Parser_API spiderMonkey的Parser API

// TODO:可以进行性能提升的地方 有

// 1. token_t完全可以用直接量来表达
// 2. 使用Object比所谓的JSTfunction快的多啊  X  确实是switch快，悲了个剧的
// 3. 性能证明charCode 与 charCodeAt差距并不大
// 4. 测试直接对比 ")" ==")"  还是判断 40=40的速度快
...
// Source transformation: node-falafel, Rocambole, Esmangle, escodegen
